...
public class Main {
    ...
    public static int maxSubarray(int[] inputCase2) {
        int maxSum = inputCase2[0];
        for (int i = 0; i < inputCase2.length-1;i++) {
            int sum = inputCase2[i];
            for (int y = i+1;y < inputCase2.length;y++) {
                sum += inputCase2[y];
                if (sum > maxSum) {
                    maxSum = sum;
                }
